<script lang="ts">
	...
	let play: (Play & { playwright_name?: string; composer_id?: number }) | null = null;
	let creator: Person | null = null;
	let creatorRole: 'playwright' | 'composer' = 'playwright';
	...
	function loadPlay() {
		try {
			play = getPlay(playId);
			if (play) {
				// Get creator details (playwright or composer)
				if (play.playwright_id) {
					creator = getPerson(play.playwright_id);
					creatorRole = 'playwright';
				} else if (play.composer_id) {
					creator = getPerson(play.composer_id);
	...
	function getMediumLabel(source: string | undefined, category: string | null, medium: string): string {
		// Bergen Phil gets its own branding
		if (source === 'bergenphilive') {
			return 'Bergen Phil Live';
		}

		// Drama series
		if (category === 'dramaserie') {
			return 'Dramaserie';
		}
		...
		// Everything else (concerts, opera, etc.)
		...
		<header class="play-header">
			<div class="header-content">
				<div class="header-text">
					...
					<p class="play-meta">
						{#if creator}
							<a href="/person/{creator.id}" class="author-link">{creator.name}</a>
							{#if creator.birth_year || creator.death_year}
								<span class="author-dates">({creator.birth_year || '?'}–{creator.death_year || ''})</span>
							{/if}
						{/if}
						{#if play.year_written}
							<span class="separator">·</span>
							<span class="year-written">{creatorRole === 'composer' ? 'komponert' : 'skrevet'} {play.year_written}</span>
						{/if}
					</p>
		...
		{#if tvPerformances.length > 0}
			<section class="performances tv-section">
				<h2>
					...
					{getMediumLabel(tvPerformances[0]?.source, play?.category, 'tv')}
				...
				{#if tvPerformances.length === 1}
					...
					<a href="/opptak/{perf.id}" class="single-perf-card">
				...
				{:else}
					...
					{#each tvPerformances as perf}
						<a href="/opptak/{perf.id}" class="performance-card-link">
		...
		{#if radioPerformances.length > 0}
			<section class="performances radio-section">
				<h2>
					...
					{getMediumLabel(radioPerformances[0]?.source, play?.category, 'radio')}
